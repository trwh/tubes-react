  ...
  return new Promise(function (resolve, reject) {
    ...
    lines.forEach((line) => {
      stationFetchPromises.push(
        fetch("https://api.tfl.gov.uk/line/" + line.id + "/stoppoints").then(
          (response) => {
            if (response.ok) {
              return response.json();
            } else {
...
function simplifyStation(station) {
  let cleanedName = station.commonName.replace(" Underground Station", "");
  return {
    id: station.id,
    name: cleanedName,
    lines: [],
...
function addLineToAnyMatchingStations(stations, stationId, line) {
  let indexOfMatchingStation = stations.findIndex(
    ...
    fetch("https://api.tfl.gov.uk/stoppoint/" + stationId + "/arrivals")
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error(
            "Error getting arrivals for " + stationId + " from TFL API.",
          );
        }
      })
      .then((json) => {
...
function simplifyArrival(arrival) {
  let regex = / Platform \d$/gim;
  let cleanedCurrentLocation = arrival.currentLocation.replace(regex, "");
  return {
    id: arrival.id,
...
function addHumanReadableTimes(arrivals) {
  arrivals.forEach((arrival) => {
    arrival.humanReadableTimeToStation = secondsToHumanReadableTime(
  ...
  return new Promise(function (resolve, reject) {
    ...
    stations.forEach((station) => {
      lineArrivalsPromises.push(
        getLineArrivals(station.id).then((lineArrivals) => {
          return lineArrivals;
        }),
      );
    });
    ...
    fetch("https://api.tfl.gov.uk/line/mode/tube/status")
      ...
      .then((json) => {
        json.forEach((lineWithStatus) => {
          allLinesWithStatuses.push(simplifyLineWithStatus(lineWithStatus));
        });
